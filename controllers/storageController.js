...
const getMonthlyEnergyConsumption = async (req, res) => {
  try {
    ...
    const monthlyData = await EnergyData.aggregate([
      ...
      },
      {
        $group: {
          ...
          lastEnergyConsumptionMeter6: {
            $first: "$energy_consumption_meter_6",
          }, // Last value for meter 6
          lastEnergyConsumptionMeter108: {
            $first: "$energy_consumption_meter_108",
          }, // Last value for meter 108
          lastEnergyConsumptionMeter201: {
            $first: "$energy_consumption_meter_201",
          }, // Last value for meter 201
        },
      },
      {
        $group: {
          _id: null, // Group everything to calculate the total sum
          totalEnergyConsumptionMeter6: {
            $sum: "$lastEnergyConsumptionMeter6",
          }, // Sum last values for meter 6
          totalEnergyConsumptionMeter108: {
            $sum: "$lastEnergyConsumptionMeter108",
          }, // Sum last values for meter 108
          totalEnergyConsumptionMeter201: {
            $sum: "$lastEnergyConsumptionMeter201",
          }, // Sum last values for meter 201
        },
      },
    ]);
    ...
    res
      .status(500)
      .json({ error: "Error fetching monthly energy consumption data" });
  }
};

const getLatestEnergySummaries = async (req, res) => {
  try {
    const records = await EnergySummary.find()
      .sort({ timestamp: -1 })
      .limit(15);
    res.json(records);
  } catch (error) {
    res.status(500).json({ error: "Error fetching energy summaries" });
  }
};
    ...
    pythonProcess.on("close", (code) => {
      ...
      try {
        ...
        if (
          ...
          predictions.next_month_prediction === undefined
        ) {
...
const predictions = async (req, res) => {
  ...
  try {
    ...
    if (!existingPrediction) {
      ...
      const predictions = await runPredictionScript(
        currentDate,
    ...
    const sanitizedPredictions = existingPrediction.nextWeekPredictions.map(
      ({ date, predicted_units }) => ({
        date,
        predicted_units,
      })
    );
